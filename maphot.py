...
def trimCatalog(cat, dcut, mcut, snrcut, shapecut):
  ...
  for i in range(len(cat['XWIN_IMAGE'])):
    ...
    xi = cat['XWIN_IMAGE'][i]
    yi = cat['YWIN_IMAGE'][i]
    dist = np.sort(((cat['XWIN_IMAGE'] - xi) ** 2 + 
                   (cat['YWIN_IMAGE'] - yi) ** 2) ** 0.5)
    d = dist[1]
    snrs = cat['FLUX_AUTO'][i] / cat['FLUXERR_AUTO'][i]
    shape = cat['AWIN_IMAGE'][i] / cat['BWIN_IMAGE'][i]
    if (cat['FLAGS'][i] == 0 
       and d > dcut 
       and m < mcut 
       and snrs > snrcut 
...
for xcat, ycat in np.array(list(zip(catalog_phot['XWIN_IMAGE'], 
  ...
  phot(xcat, ycat, radius=fwhm * roundAperRad, l=0.0, a=0.0, exptime=EXPTIME,
       #zpt=26.0, skyRadius=4 * fwhm, width=np.min([6 * fwhm, 30.]), 
